...
\[
    \mathcal{F}(D, \theta) = \mathrm{KL}[q(w | \theta) || P(w)] - \mathbb{E}_{q(w | \theta)}[\log P(D | w)]
\]
...
\[
    \frac{1}{M} \mathrm{KL}[q(w | \theta) || P(w)]
\]

where $M$ is the number of minibatches; the second one is

\[
    \pi_i \mathrm{KL}[q(w | \theta) || P(w)]
\]
...
In this section I will describe the reproduction of the MNIST classification experiment proposed in the paper. The code is available on GitHub at \url{https://github.com/Maestro-Zacht/weight-uncertainty-nns/blob/main/experiment.ipynb}.
